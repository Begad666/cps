...
public class ChangePingData implements Listener {
    ...
    public void onPing(ProxyPingEvent event) {
        ...
        if (Core.getConfig().get().getBoolean("settings.maintenance-enabled")) {
            ...
            serverPing.setPlayers(new Players(0, 0, Utils.getMaintenanceHoverMessage()));
            ...
            serverPing.setPlayers(new Players(Core.getConfig().get().getInt("network-info.max-players"), ProxyServer.getInstance().getOnlineCount(), Utils.getHoverMessage()));
